...
| Path | Contents |
|---|---|
| `apps/` | Application entry points. The current target is `blood_flow` from `blood_flow.cc`. |
| `include/` | Public headers, including the `BloodFlowSystem` declaration and numerical data structures. |
...
| `NumData/`, `notebooks/` | Additional data and exploratory/reference material; their presence is not a validation claim. |
