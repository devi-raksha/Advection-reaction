...
namespace dealii
{
    ...
    void BloodFlowSystem<dim, spacedim>::assemble_system()
    {
            ...
            for (unsigned int point = 0; point < fe_v.n_quadrature_points; ++point)
            {
                ...
                for (unsigned int i = 0; i < n_dofs; ++i)
                {
                    for (unsigned int j = 0; j < n_dofs; ++j)
                    {
                        ...
                        const double b_gradA = b_vec * fe_v[area_extractor].gradient(i, point);
                        copy_data.cell_matrix(i, j) -=
                            0.5 * (fe_v[area_extractor].value(j, point) * U_old * b_gradA + fe_v[velocity_extractor].value(j, point) * A_old * b_gradA) *
                            JxW[point];
                    ...
                    copy_data.cell_rhs(i) +=
                        (1.0 / rho) * P_old * b_vec * fe_v[velocity_extractor].gradient(i, point) * JxW[point];
                               ...
                               BloodFlowCopyData &copy)
        {
            ...
            fe_iv.get_fe_face_values(0)[area_extractor].get_function_values(solution_old, A_L);
            fe_iv.get_fe_face_values(0)[velocity_extractor].get_function_values(solution_old, U_L);
            fe_iv.get_fe_face_values(1)[area_extractor].get_function_values(solution_old, A_R);
            fe_iv.get_fe_face_values(1)[velocity_extractor].get_function_values(solution_old, U_R);

            fe_iv.get_fe_face_values(0)[area_extractor].get_function_values(solution, A_L_im);
            fe_iv.get_fe_face_values(0)[velocity_extractor].get_function_values(solution, U_L_im);
            fe_iv.get_fe_face_values(1)[area_extractor].get_function_values(solution, A_R_im);
            fe_iv.get_fe_face_values(1)[velocity_extractor].get_function_values(solution, U_R_im);
            ...
            for (unsigned int q = 0; q < n_q; ++q)
            {
                ...
                const double b_dot_n = compute_tangent_normal_product<dim, spacedim>(cell, normal);
                ...
                const auto [flux_A, flux_U] = compute_numerical_flux_rusanov_semi_implicit<dim, spacedim>(
                    AL, UL, AR, UR, AL_im, UL_im, AR_im, UR_im,
                    PL, PR, cL_im, cR_im, rho, b_dot_n);

                // Penalty parameter
                const double h_face = cell->measure();
                const double sigma = compute_penalty_parameter<dim, spacedim>(cL, cR, h_face, theta);
                                   ...
                                   BloodFlowCopyData &copy)
        {
            ...
            for (unsigned int q = 0; q < n_q; ++q)
            {
                const double b_dot_n = compute_tangent_normal_product<dim, spacedim>(cell, normals[q]);
                ...
                const auto [flux_A, flux_U] = compute_numerical_flux_rusanov<dim, spacedim>(
                    AL, UL, AR, UR, PL, PR, cL, cR, rho, b_dot_n);

                // Physical fluxes for RHS
                const double FA_R = compute_physical_area_flux<dim, spacedim>(AR, UR, b_dot_n);
                const double FU_R = compute_physical_momentum_flux<dim, spacedim>(UR, PR, rho, b_dot_n);
                ...
                for (unsigned int i = 0; i < fe_face.get_fe().dofs_per_cell; ++i)
                {
                    ...
                    copy.cell_rhs(i) +=
                        -(FA_R * fe_face[area_extractor].value(i, q) +
                          FU_R * fe_face[velocity_extractor].value(i, q)) *
                        JxW[q];
                }
            }
        };
        ...
        for (unsigned int cycle = 0; cycle < n_refinement_cycles; ++cycle)
        {
            ...
            for (unsigned int step = 1; step <= n_time_steps; ++step)
            {
                ...
                else
                {
                    ...
                    PreconditionSSOR<> preconditioner;
                    const double omega = 1.4; // SSOR relaxation parameter (typical range: 1.0-2.0)
                    preconditioner.initialize(system_matrix_time, omega);
                    gmres.solve(system_matrix_time, solution, tmp_vector, preconditioner);
