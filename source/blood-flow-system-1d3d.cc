...
namespace dealii
{
    ...
    double compute_wave_speed(const double area, const double reference_area,
                              const double elastic_modulus, const double density)
    {
    ...
    template <int dim, int spacedim>
    double compute_pressure(const double area, const double reference_area,
                            const double elastic_modulus, const double reference_pressure)
    {
        const double ratio = area / reference_area;
        const double m = 0.5; // tube law exponent
    ...
    {
        BloodFlowScratchData(const FiniteElement<dim, spacedim> &fe,
    ...
    template <int dim, int spacedim>
    void BloodFlowSystem<dim, spacedim>::initialize_params(const std::string &filename)
    {
    ...
    template <int dim, int spacedim>
    void BloodFlowSystem<dim, spacedim>::setup_system()
    {
        if (!fe)
        {
    ...
    template <int dim, int spacedim>
    void BloodFlowSystem<dim, spacedim>::assemble_system()
    {
                                     ...
                                     BloodFlowCopyData &copy_data)
        {
            ...
            for (unsigned int point = 0; point < fe_v.n_quadrature_points; ++point)
            {
                ...
                const double A_old = old_solution_values[point][0]; // Area at previous time
                const double U_old = old_solution_values[point][1]; // Velocity at previous time
                ...
                for (unsigned int i = 0; i < n_dofs; ++i)
                {
                    ...
                    for (unsigned int j = 0; j < n_dofs; ++j)
                    {
                        ...
                        if (component_i == 0 && component_j == 0) // A-A block
                        {
                            ...
                            // This becomes: A_old * b·\nabla U (since A_old is constant)
                            const double adv_term = A_old * (b_vec * grad_i);
                            copy_data.cell_matrix(i, j) += adv_term * fe_v.shape_value(j, point) * JxW[point];
                        }
                        else if (component_i == 0 && component_j == 1) // A-U block
                        {
                            // Semi-implicit: A_old * b·\nabla U
                            const double adv_term = A_old * (b_vec * fe_v.shape_grad(j, point));
                            copy_data.cell_matrix(i, j) += adv_term * fe_v.shape_value(i, point) * JxW[point];
                        }
                        ...
                        else if (component_i == 1 && component_j == 1) // U-U block
                        {
                            ...
                            const double nonlinear_conv = U_old * (grad_i[0] * fe_v.shape_value(j, point));

                            // Reaction term: cU
                            const double reaction = viscosity_c * fe_v.shape_value(i, point) *
                                                    fe_v.shape_value(j, point);

                            copy_data.cell_matrix(i, j) += (nonlinear_conv + reaction) * JxW[point];
                        }
                        else if (component_i == 1 && component_j == 0) // U-A block (pressure term)
                        {
                            ...
                            const double pressure_grad = (1.0 / rho) * dpda *
                                                         (grad_i[0] * fe_v.shape_value(j, point)); // Use [0] component
                                     ...
                                     BloodFlowCopyData &copy_data)
        {
            ...
            std::vector<Vector<double>> old_solution_face_values(q_points.size(), Vector<double>(2));
            fe_iv.get_fe_face_values(0).get_function_values(solution_old, old_solution_face_values);
            ...
            for (unsigned int qpoint = 0; qpoint < q_points.size(); ++qpoint)
            {
                ...
                for (unsigned int i = 0; i < n_dofs; ++i)
                {
                    ...
                    const unsigned int component_i = fe->system_to_component_index(dof_indices_i[0]).first;

                    for (unsigned int j = 0; j < n_dofs; ++j)
                    {
                        const auto dof_indices_j = fe_iv.interface_dof_to_dof_indices(j);
                        const unsigned int component_j = fe->system_to_component_index(dof_indices_j[0]).first;

                        // Interface terms for the coupled system
                        if (component_i == component_j)
                        {
                            // Upwind stabilization for both equations
                            const double stabilization = theta * abs_b_dot_n *
                            ...
                            {
                                consistency = A_old * b_dot_n *
                                              fe_iv.average_of_shape_values(j, qpoint) *
                                              fe_iv.jump_in_shape_values(i, qpoint);
                            }
                            else // Velocity equation
                            {
                                consistency = U_old * b_dot_n *
                                              fe_iv.average_of_shape_values(j, qpoint) *
                                              fe_iv.jump_in_shape_values(i, qpoint);
                            }

                            copy_data_face.cell_matrix(i, j) += (consistency + stabilization) * JxW[qpoint];
                        }
                    }
                }
            }
                ...
                if (b_dot_n < 0) // Inflow boundary
                {
                    for (unsigned int i = 0; i < n_facet_dofs; ++i)
                    {
                        ...
                        for (unsigned int j = 0; j < n_facet_dofs; ++j)
                        {
                            ...
                            if (component_i == component_j)
                            {
                                copy_data.cell_matrix(i, j) += theta * abs_b_dot_n *
                        ...
                        if (component_i == 0) // Area boundary condition
                        {
                            ...
                            copy_data.cell_rhs(i) += -theta * abs_b_dot_n * bc_value *
    ...
    {
        const std::string filename = output_filename + "-" + std::to_string(cycle) + ".vtk";
        ...
        std::vector<DataComponentInterpretation::DataComponentInterpretation>
            data_component_interpretation(2, DataComponentInterpretation::component_is_scalar);
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
        {
            ...
            for (unsigned int i = 0; i < fe->n_dofs_per_cell(); ++i)
            {
                ...
                if (component == 0) // Area component
                {
                    ...
                    pressure[dof_indices[i]] = dealii::compute_pressure<dim, spacedim>(
                        area, reference_area, elastic_modulus, reference_pressure);
                }
            }
        }
    }
    ...
    void BloodFlowSystem<dim, spacedim>::run()
    {
        ...
        {
        public:
            InitialCondition() : Function<spacedim>(2) {} // 2 components
        ...
        for (unsigned int step = 1; step <= n_time_steps; ++step)
        {
            ...
            // Assemble system matrix (depends on solution_old for semi-implicit terms)
            assemble_system();
